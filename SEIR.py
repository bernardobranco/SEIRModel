...
if __name__ == "__main__":
    ...
    country = "Italy"
    pop = 58500000
    data_loc = train_df[train_df["Country_Region"] == country].query("ConfirmedCases > 0")
    if len(data_loc) == 0:
        train_loc = data_loc[data_loc["Province_State"] == country].query(
            "ConfirmedCases > 0"
        )
    n_infected = data_loc["ConfirmedCases"].iloc[0]
    train_loc = data_loc[:-7]
    valid_loc = data_loc[-7:]
    ...
    model = SEIR(pop, n_infected, t_inc, t_inf)
    # model.solve(max_days, R_n)
    # model.plot(train_loc)

    # initial_guess = [R_n, cfr, k, L]
    # res_inf, res_fat, res_avg = model.eval(
    #     initial_guess,
    #     train_loc["ConfirmedCases"],
    #     train_loc["Fatalities"],
    #     decay_func="hill",
    #     optim_days=20,
    #     weight_data=True,
    # )
    # print(f"MSLE infected: {res_inf}")
    # print(f"MSLE fatalities: {res_fat}")
    # print(f"MSLE average: {res_avg}")

    model.fit(
        y_inf=train_loc["ConfirmedCases"],
        y_fat=train_loc["Fatalities"],
        optim_days=30,
        optim_field="avg",
    )
    # model.plot(data_inf=data_loc["ConfirmedCases"], data_fat=data_loc["Fatalities"], num_train_days=len(train_loc))

    _, _, inf, rec = model.predict(num_train_days=len(train_loc), forecast_days=len(valid_loc))
    res = model.eval(pred_inf=inf, pred_rec=rec, y_inf=valid_loc["ConfirmedCases"], y_fat=train_loc["Fatalities"])
    print(res)
    print(len(inf))
    print(len(valid_loc))
    ...
    sus, exp, inf, rec = model.predict(num_train_days=len(train_loc), forecast_days=100)
    model.plot(data_inf=train_loc["ConfirmedCases"], data_fat=train_loc["Fatalities"], num_train_days=len(train_loc))

    # model.plot(
    #     data_inf=train_loc["ConfirmedCases"],
    #     data_fat=train_loc["Fatalities"],
    #     forecast_days=False,
    # )
    ...
    #     return_params="all",
    # )
    # model.plot_decay_curve(num_days=len(train_loc))
    # model.plot(data_inf=train_loc["ConfirmedCases"], data_fat=train_loc["Fatalities"])
